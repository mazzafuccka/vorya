       ...
       foreach ($search_home_blocks as $key => $value) {
                ...
                case 'location': $imic_country_wise_city = imic_get_multiple_city();
                        if (!empty($imic_country_wise_city)) {
                            $output .='<div class="search-field"><label>'.__('Location', 'framework').'</label><select name="propery_location" class="form-control selectpicker">
...
echo '<ul class="owl-carousel owl-alt-controls" data-columns="4" data-autoplay="no" data-pagination="no" data-arrows="yes" data-single-item="no" '.$data_rtl.'>';
                              while(have_posts()):the_post(); 
							  $property_images = get_post_meta(get_the_ID(),'imic_property_sights',false);
								$total_images = count($property_images);
